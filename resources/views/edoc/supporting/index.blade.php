...
<ul class="list-inline menu-left mb-0">
    ...
    <li>
        <div class="page-title-box">
            <h4 class="page-title">Master Supporting </h4>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('dashboard_edoc.index') }}">Beranda</a></li>
                <li class="breadcrumb-item active">Master Supporting</li>
            </ol>
        </div>
    </li>
...
<div class="card">
        ...
        <div class="row">
            <div class="col-sm-6">
                <form class="form-inline" action="{{ url()->current() }}" >
                    ...
                    <a href="{{ route('supporting.index') }}" class="btn btn-danger"><i class="fa fa-trash"></i></a>
                </form>
            </div>
            <div class="col-sm-6">
			@if(Auth::user()->hasAnyPermission([1,7,21]))
                <a href="{{ route('supporting.create') }}" class="btn btn-icon waves-effect waves-light btn-primary float-right"> <i class="fa fa-plus"></i> </a>
			@endif
            </div>
        </div>
    </div>
    <div class="card-body">
        <table class="table mb-0">
            ...
            <tbody>
            @forelse ($supportings as $r)
                <tr>
                    <td>{{ $r->id }}</td>
                    <td>{{ $r->title }}</td>
                    <td class="text-center">{{ $r->revisi }}</td>
                    <td class="text-center">
                    @if(Auth::user()->hasAnyPermission([1,7,22]))
                        @if($r->status == 'Active') 
                        <a href="{{ route('supporting.status', ['id' => $r->id, 'status' => 'Deactive']) }}" class="btn btn-success waves-effect waves-light btn-sm">Active</a>
                        @else  
                        <a href="{{ route('supporting.status', ['id' => $r->id, 'status' => 'Active']) }}" class="btn btn-danger waves-effect waves-light btn-sm">Deactive</a>
                        @endif
					@else
						@if($r->status == 'Active') 
                    ...
                    <td class="text-center">
						@if(Auth::user()->hasAnyPermission([1,7,22]))
                        <a href="{{ route('supporting.download', $r->id) }}" class="btn btn-sm btn-primary">
                            <i class="fa fa-download"></i>
                        </a>
                        <button data-toggle="modal" data-id="{{ $r->id }}" data-desc="{{ $r->title }}" data-filepath="{{ $r->filepath }}" data-rev="{{ $r->revisi }}" data-jenis="{{ $r->jenis }}" data-target="#md_edit_file" class="btn btn-sm btn-warning">
                            <i class="fa fa-upload"></i>
                        </button>
                        <button data-toggle="modal" data-id="{{ $r->id }}" data-desc="{{ $r->title }}" data-target="#md_del" class="btn btn-sm btn-danger">
                            <i class="fa fa-trash"></i>
                        </button>
						@elseif(Auth::user()->hasAnyPermission([20]))
							@if($r->status == 'Active')
                                <a href="{{ route('supporting.download', $r->id) }}" class="btn btn-sm btn-primary">
                                    <i class="fa fa-download"></i>
                                </a>
								<a href="{{ route('list.create', ['id' => $r->id, 'doc_type' => $r->jenis, 'iso_type' => 'Supporting']) }}" class="btn btn-sm btn-warning">
    ...
    <div class="modal-dialog modal-dialog-popout" role="document">
        <div class="modal-content">
            <div class="modal-header bg-danger">
                <h4 class="modal-title text-white">Hapus Supporting Document</h4>
            ...
            <div class="modal-body">
                ...
                @csrf
                    <div class="form-group row">
                        <div class="col-lg-12">
                            Anda yakin akan menghapus supporting document <span id="data_iso"></span> ?
                        </div>
                    </div>
            </div>
    ...
    <div class="modal-dialog modal-dialog-popout" role="document">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h4 class="modal-title text-white title-ubah-file">Ubah File Supporting Document </h4>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
            </div>
            <div class="modal-body">
                <form method="POST" action="{{ route('supporting.update_file') }}" enctype="multipart/form-data" id="file-form">
                @csrf
...
@section('script')
    <script>
        ...
        $("#md_del").on("show.bs.modal", function(e) {
            ...
            $('#data-form').attr('action', "{{ url('/') }}/edoc/supporting/"+id);
        })
