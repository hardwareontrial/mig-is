...
<div class="container-fluid">
    ...
    <div class="card m-b-30">
                ...
                <div class="col-md-6">
                    @if(Auth::user()->hasAnyPermission([1,24]) || $schedule->created_by == Auth::user()->id)
                        @if (date("Y-m-d H:i:s") < $schedule->date_end)
                            <button type="button" data-toggle="modal" data-target="#md_add_participant" 
                                class="btn btn-icon waves-effect waves-light btn-primary  float-right" 
                    ...
                    @if (!empty($participant_access))
                        <a href="{{ route('exam.show',$schedule->id) }}" class="btn btn-icon waves-effect waves-light btn-primary float-right" hidden> 
                            <i class="fa fa-play"></i> 
                        </a>
                        <a href="{{ route('exam.start',['id'=>$schedule->id,'last_page'=>!empty($exam_page_position->page) ? $exam_page_position->page : '1']) }}" class="btn btn-icon waves-effect waves-light btn-primary float-right"> 
                            <i class="fa fa-play"></i> 
                        </a>
                    @endif
                </div>
        ...
        <div class="card-body">
			...
			@endif
            <table class="table table-hover table-border" id="tblScheduleLists">
                <thead>
                    <?php
                    ...
                    <tr>
                        ...
                        <th class="text-center">User Result</th>
                ...
                <tbody>                    
                    ...
                    <tr>
                        <td style="display:none;" id="ScheduleId">{{$r->schedule_id}}</td>                        
                        ...
                        <td align="center">
                        @if(Auth::user()->hasAnyPermission([1,24]) || $schedule->created_by == Auth::user()->id)
                            <button class="btn btn-sm btn-info" id="btnUserAnswer">
						...
						<td align="center">
                            <button class="btn btn-sm btn-danger btn-circle" data-id="{{ $r->id }}" data-desc="{{ '['.$r->nik.'] '.$r->user['name'] }}" data-toggle="modal" data-target="#md_del_participant">
...
<div class="modal fade" id="md_result_user" role="dialog" aria-labelledby="modal-popout" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-popout" role="document">
        <div class="modal-content">
            <div class="modal-header bg-info">
                <h4 class="modal-title text-white">Hasil Jawaban Peserta</h4>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
            </div>
            <div class="modal-body">
                <div class="table-responsive">
                    <table class="table table-stiped" id="tableAnswersLists">
                        <thead>
                            <th style="width:5%;">No.</th>
                            <th style="width:30%;">Soal</th>
                            <th style="width:30%;">Jawaban User</th>
                            <th style="width:30%;">Jawaban Benar</th>
                            <th style="width:5%;">Koreksi</th>                            
                        </thead>
                        <tbody>                            
                        </tbody>
                    </table>
...
@section('script')
    <script>
        ...
        $("#tblScheduleLists > tbody > tr").on('click',"#btnUserAnswer", function(){

            $("#tableAnswersLists").find("tbody tr").html("");

            var xRow = $(this).closest("tr");
            var xId = xRow.find("td:nth-child(1)").text();
            var xNik = xRow.find("td:nth-child(2)").text();
            var xhtml = "";
            var no = 1;             
            $.ajax({
                type: "GET",
                url: location.origin+"/okm/user/answer/"+xId+"/"+xNik,
                success: function(data) {                     
                    ...
                    for(var i in data){
                        ...
                        xhtml += "<tr "+style+">";                        
                        xhtml += "<td>"+ no +"</td>";
                        xhtml += "<td>"+data[i].questions+"</td>";
                        if(data[i].user_answer == "" || data[i].user_answer == null){
                            xhtml += "<td><label class='badge badge-square badge-danger'>Belum terjawab</label></td>";
                        }else{
                            xhtml += "<td>"+data[i].user_answer+"</td>";
                        }                                                
                        xhtml += "<td>"+data[i].right_answer+"</td>";
                        if(data[i].status == true){
                            xhtml += "<td><span class='badge badge-success'><i class='fa fa-check'></i></span></td>";
                        }else if(data[i].status==false){
                            xhtml += "<td><span class='badge badge-danger'><i class='fa fa-close'></i></span></td>";
                        }
                        xhtml += "</tr>";
                    }

                    $("#md_result_user #tableAnswersLists").find("tbody").append(xhtml);
                }                
            });  
            $("#md_result_user").modal("show");                                 
        });
    </script>
@stop
