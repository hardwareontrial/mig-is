                ...
                <div class="col-sm-6">
					@if(Auth::user()->hasAnyPermission([1,24,39]))
                        @if($collection->created_by == Auth::user()->id)
                            <a href="{{ route('question.edit',$collection->id) }}" class="btn btn-icon waves-effect waves-light btn-warning  float-right">
        ...
        <div class="card-header">
            <div class="row">
                ...
                @if(Auth::user()->hasAnyPermission([1,24,32]))
                    ...
                    <button type="button" data-toggle="modal" data-target="#md_new_content" class="btn btn-icon waves-effect waves-light btn-primary  float-right">
				...
				@if(Auth::user()->hasAnyPermission([1,24,32]))
                    @if(Auth::user()->id == $collection->created_by)
                    <button type="button" data-toggle="modal" data-target="#md_del_all" class="btn btn-icon waves-effect waves-light btn-danger  float-right">
                ...
                @if(Auth::user()->hasAnyPermission([1,24,32]))
                    @if(Auth::user()->id == $collection->created_by)
                    <button type="button" data-toggle="modal" data-target="#md_import" class="btn btn-icon waves-effect waves-light btn-success  float-right" style="margin-right:10px;">
        ...
        <div class="card-body">                        
            <table class="table table-bordered table-vcenter" hidden>
                <thead>
                    <tr>
                        ...
                        <th colspan="4" class="text-center" style="display:none;">Jawaban</th>
			...
			<table class="table table-bordered table-vcenter">
                <thead>                    
                    ...
                    <th colspan="2" class="text-center" style="width:25%">Jawaban</th>
                ...
                <tbody>
                    @php 
                        ...
                        $created_user = $collection->created_by;
                        $rowspan = '';
                    @endphp                    
                    @forelse($contents as $r)
                        @php
                            if(count($r->answers) == 0 || $auth_user != $created_user){
                                $rowspan = '1';
                            }else{
                                $rowspan = '5';
                            }
                        @endphp
                    <tr>                      
                        <th rowspan="{{$rowspan}}" class="text-center" 
                            style="vertical-align : middle;text-align:center;">{{ $n }}</th>
                        <td rowspan="{{$rowspan}}" class="text-center" 
                            style="vertical-align:middle;width:30%;">{{ $r->question }}</td>                        
                        @if($collection->created_by != Auth::user()->id)
                            <td style="vertical-align:middle; text-align:center;" rowspan="{{$rowspan}}" colspan="3">
                                <span class="badge badge-warning">You're not authorized!</span>
                            </td>                        
                        @elseif($collection->created_by == Auth::user()->id)                        
                            <?php $p = 1; ?>
                            @forelse($r->answers as $a)
                                <tr class="text-center">
                                    <td style="vertical-align:middle;width:10%;" 
                                        class="@if ($a->answer_key == 1)  table-success @endif">
                                        Pilihan {{ $p }}
                                    </td>
                            ...
                            @empty                                
                                <td rowspan="{{$rowspan}}" colspan='3' class="text-center" style="vertical-align : middle;text-align:center;">                                    
                                    Tidak Ada Data
                                </td>
                            @endforelse                                          
                        @endif
                    ...
                    @empty
                        <tr>
                            <td colspan="3" rowspan="5" class="text-center" 
                                style="vertical-align : middle;text-align:center;">Tidak ada data</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<div class="modal fade" id="md_del_all" role="dialog" aria-labelledby="modal-popout" aria-hidden="true">
    <div class="modal-dialog modal-dialog-popout" role="document">
        <div class="modal-content">
            <div class="modal-header bg-danger">
                <h4 class="modal-title text-white">Hapus Semua Pertanyaan</h4>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
            </div>
            <div class="modal-body">
                <form method="POST" action="" id="content-del-form">
                ...
                @csrf
                    <div class="form-group row">
                        <div class="col-lg-12">
                            Anda yakin akan menghapus semua pertanyaan berikut ini<span id="content_id"></span> ?
                        </div>
                    </div>
            </div>
            <div class="modal-footer">
...
@section('script')
    <script>
        ...
        $("#md_del_all").on("show.bs.modal", function(e) {
            var id =  $(e.relatedTarget).data('qid');
            var question =  $(e.relatedTarget).data('question');
